...
func generateContent(g *protogen.GeneratedFile, file *protogen.File) {
	...
	// Track whether we need to import sessionPackage
	needSessionPackage := false
	for _, service := range file.Services {
		for _, method := range service.Methods {
			...
			if !ok || methodOptions == nil {
				needSessionPackage = true
				break
			}

			if ok && methodOptions != nil && !methodOptions.Rpc {
				needSessionPackage = true
				break
			}
		}

		if needSessionPackage {
			break
		}
	}

	if needSessionPackage {
		g.QualifiedGoIdent(sessionPackage.Ident(""))
	}
